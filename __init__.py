...
class CameraviewPaint(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        select_mat = obj.data.materials[0].texture_slots[0].texture.image.size[:]
...
class ArtistPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator("scene.create_brush_scene",
                text="Create Brush Maker Scene", icon='OUTLINER_OB_CAMERA')
        ...
        row.operator("image.cameraview_paint",
                    text = "Add Painting Camera", icon = 'RENDER_REGION')
